...
void Preprocess::avia_handler(const livox_ros_driver2::CustomMsg::ConstPtr &msg)
{
  ...
  for(uint i=1; i<plsize; i++)
  {
    if((msg->points[i].line < N_SCANS) && ((msg->points[i].tag & 0x30) == 0x10 || (msg->points[i].tag & 0x30) == 0x00))//只取第0或1个回波
    {
      valid_num ++;
      if (valid_num % point_filter_num == 0)//下采样，但point_filter_num=1未使用
      {
        pl_full[i].x = msg->points[i].x;
        pl_full[i].y = msg->points[i].y;
        pl_full[i].z = msg->points[i].z;
        pl_full[i].intensity = msg->points[i].reflectivity; // z; //
        pl_full[i].curvature = msg->points[i].offset_time / float(1000000); // 使用曲率作为每个激光点的时间，曲率单位：ms 
        ...
        if(((abs(pl_full[i].x - pl_full[i-1].x) > 1e-7) 
            ...
            || (abs(pl_full[i].z - pl_full[i-1].z) > 1e-7)))
        {
          pl_surf.push_back(pl_full[i]);//满足在一定范围且不与上一帧重合，装入pl_surf
        }
      }
    }
  }
