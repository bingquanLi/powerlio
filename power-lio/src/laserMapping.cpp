...
geometry_msgs::PoseWithCovarianceStamped my_pose;//改动的
...
void publish_frame_body(const ros::Publisher & pubLaserCloudFull_body)
{
    ...
    laserCloudmsg.header.stamp = ros::Time().fromSec(lidar_end_time);
    laserCloudmsg.header.frame_id = "lidar_power";
...
// {
//     Eigen::Quaterniond quat(kf_output.x_.rot);
//     tf::Quaternion qRaw = tf::Quaternion(quat.coeffs()[0], quat.coeffs()[1], quat.coeffs()[2], quat.coeffs()[3]);
//     double rollRaw, pitchRaw, yawRaw;
//     tf::Matrix3x3(qRaw).getRPY(rollRaw, pitchRaw, yawRaw);
//     //std::cout << (yawRaw/M_PI)*180 << std::endl;
//     if (!firstReadImu)
//     {
//         firstReadImu = true;
//         initYaw = yawRaw;
//     }
//     yawRaw -= initYaw;
//     // 将校正后的偏航角限制在[-pi, pi]范围内
//     if (yawRaw > M_PI) {
//         yawRaw -= 2 * M_PI;
//     } else if (yawRaw < -M_PI) {
//         yawRaw += 2 * M_PI;
//     }
//     auto ori = tf::createQuaternionMsgFromRollPitchYaw(rollRaw, pitchRaw, yawRaw);
//     //std::cout <<" "<<" new"<< (yawRaw/M_PI)*180 << std::endl;  
//     if (!use_imu_as_input)
//     {
...
void set_posestamp(T & out)
{
    ...
    if (!use_imu_as_input)
    {
        ...
        out.orientation.x = ori.x;
        out.orientation.y = ori.y;
        out.orientation.z = ori.z;
        out.orientation.w = ori.w;
        
        double roll{}, pitch{}, yaw = initYaw;
        auto qRotation = tf::createQuaternionMsgFromRollPitchYaw(roll, pitch, yaw);
        Eigen::Quaterniond qNew;
        qNew.coeffs()[0] = qRotation.x;
        qNew.coeffs()[1] = qRotation.y;
        qNew.coeffs()[2] = qRotation.z;
        qNew.coeffs()[3] = qRotation.w;
 	    Eigen::Matrix3d rotation_matrix = qNew.toRotationMatrix();
 	    Eigen::Vector3d v = Eigen::Vector3d(kf_output.x_.pos(0), kf_output.x_.pos(1), kf_output.x_.pos(2));
 	    v = rotation_matrix.transpose() * v;
        //v = rotation_matrix * v;
 	    out.position.x = v[0];
 	    out.position.y = v[1];
 	    out.position.z = v[2];
        //out.position.z += 0.115; 
    }
    else
    {
        out.orientation.x = ori.x;
...
void publish_odometry(const ros::Publisher & pubOdomAftMapped)
{
    ...
    br.sendTransform( tf::StampedTransform( transform, odomAftMapped.header.stamp, "camera_init", "lidar_power") );
}
...
//添加的
void publish_my_pose(const ros::Publisher pub_my_pose)
{
    set_posestamp(my_pose.pose.pose);
    // msg_body_pose.header.stamp = ros::Time::now();
    //my_pose.header.stamp = ros::Time().fromSec(lidar_end_time);
    if (publish_odometry_without_downsample)
    {
        my_pose.header.stamp = ros::Time().fromSec(time_current);
    }
    else
    {
        my_pose.header.stamp = ros::Time().fromSec(lidar_end_time);
    }
    my_pose.header.frame_id = "odom";

    //static int jjj = 0;
    //jjj++;
    // if (jjj % 2 == 0) // if path is too large, the rvis will crash
    //{
        //my_pose.pose.emplace_back(msg_pose);
        pub_my_pose.publish(my_pose);
    //}
}
int main(int argc, char** argv)
{
    ...
    memset(point_selected_surf, true, sizeof(point_selected_surf));//point_selected_surf[100000]全为true
    downSizeFilterSurf.setLeafSize(filter_size_surf_min, filter_size_surf_min, filter_size_surf_min);//下采用叶子0.5
    ...
    ros::Publisher pubPath          = nh.advertise<nav_msgs::Path> 
            ("/path", 1000);
    ros::Publisher pub_my_pose      = nh.advertise<geometry_msgs::PoseWithCovarianceStamped> 
            ("/my_pose", 1000);
    // ros::Publisher plane_pub = nh.advertise<visualization_msgs::Marker>
            // ("/planner_normal", 1000);
//------------------------------------------------------------------------------------------------------
    signal(SIGINT, SigHandle);//检测到Ctrl+c退出，flg_exit=true
        ...
        if(sync_packages(Measures)) 
        {
            ...
            if (flg_first_scan)//需要
            {
                ...
                if(imu_en)
                {
                    ...
                    kf_input.x_.gravity << VEC_FROM_ARRAY(gravity);//#define VEC_FROM_ARRAY(v)  v[0],v[1],v[2]      gravity: [0.0, 0.0, -9.810]
                    kf_output.x_.gravity << VEC_FROM_ARRAY(gravity);
                    //cout << "time:" << kf_input << endl;
            ...
            double old;
            t0 = omp_get_wtime();
            // cout << "time:" << t0-old << endl;
            // old=t0;
            /*** downsample the feature points in a scan ***/
            t1 = omp_get_wtime();
            p_imu->Process(Measures, feats_undistort);//初始化imu，记录偏置mean_acc，mean_gyr 将lidar点云装到feats_undistort
            if(space_down_sample)//launch true体素下采样(pcl)，
            {
                downSizeFilterSurf.setInputCloud(feats_undistort);
                downSizeFilterSurf.filter(*feats_down_body);
                sort(feats_down_body->points.begin(), feats_down_body->points.end(), time_list); 
                //在排序完成后，feats_down_body->points 将按点的 curvature 值从小到大排列。
            }
            else
            {
                cout << "else"<< endl;
            ...
            {//需要
                time_seq = time_compressing<int>(feats_down_body);//记录错误的时间戳
                feats_down_size = feats_down_body->points.size();//下采样后的数量
            }

            if (!p_imu->after_imu_init_) // !p_imu->UseLIInit && 利用重力初始化姿态
            {
                if (!p_imu->imu_need_init_)
                { 
                    V3D tmp_gravity;//三维向量
                    if (imu_en)
                    {
                        tmp_gravity = - p_imu->mean_acc / p_imu->mean_acc.norm() * G_m_s2;}//记录开始时刻的重力向量
                    else
                    ...
                    M3D rot_init;
                    p_imu->Set_init(tmp_gravity, rot_init);//得到实际重力与理论间的旋转矩阵
                    //卡尔曼部分
                    kf_input.x_.rot = rot_init;
                    kf_output.x_.rot = rot_init;
                    // kf_input.x_.rot; //.normalize();
                    // kf_output.x_.rot; //.normalize();
                    kf_output.x_.acc = - rot_init.transpose() * kf_output.x_.gravity;//实际加速度(静止)
                }
                else{
                continue;}
            }
            ...
            if(!init_map)
            {
                feats_down_world->resize(feats_undistort->size());//设置世界坐标系下点云数量等于未下采样的点云
                for(int i = 0; i < feats_undistort->size(); i++)//遍历点云,
                {
                    {
                        pointBodyToWorld(&(feats_undistort->points[i]), &(feats_down_world->points[i]));//将点云转到世界坐标系下
                    }
                }
                for (size_t i = 0; i < feats_down_world->size(); i++)//遍历世界坐标系下的点云 
                {
                    init_feats_world->points.emplace_back(feats_down_world->points[i]);
                }
                if(init_feats_world->size() < init_map_size) //init_map_size = 10，点太少初始化失败
                {init_map = false;}
                else
                {   
                    ivox_->AddPoints(init_feats_world->points);//增量式体素结构iVox
            ...
            normvec->resize(feats_down_size);//PointCloudXYZI::Ptr
            feats_down_world->resize(feats_down_size);//PointCloudXYZI::Ptr

            Nearest_Points.resize(feats_down_size);//std::vector<PointVector>

            t2 = omp_get_wtime();
            
            /*** iterated state estimation 迭代状态估计***/
            crossmat_list.reserve(feats_down_size);//std::vector<M3D> 点云位置向量对应的反对称矩阵
            pbody_list.reserve(feats_down_size);//std::vector<V3D> 点云位置(x,y,z)序列
            // pbody_ext_list.reserve(feats_down_size);
                          
            for (size_t i = 0; i < feats_down_body->size(); i++)//遍历下采样后的点云
            {
                V3D point_this(feats_down_body->points[i].x,
                            feats_down_body->points[i].y,
                            feats_down_body->points[i].z);
                pbody_list[i]=point_this;
                if (!extrinsic_est_en)//对于攻击性运动，将此变量设置为false
                // {
                //     if (!use_imu_as_input)
                //     {
                ...
                // else
                {
                    point_this = Lidar_R_wrt_IMU * point_this + Lidar_T_wrt_IMU;//转到imu坐标系
                    M3D point_crossmat;
                    point_crossmat << SKEW_SYM_MATRX(point_this);//构造point_this的反对称矩阵
                    crossmat_list[i]=point_crossmat;
                }
            }
            //重要过程
            if (!use_imu_as_input)// use_imu_as_input=0
            {     
                ...
                {
                    PointType &point_body  = feats_down_body->points[idx+time_seq[k]];//跳过时间错误的点

                    time_current = point_body.curvature / 1000.0 + pcl_beg_time;//当前点的时间

                    if (is_first_frame)//第一帧进入
                    {
                        if(imu_en)
                        {
            ...
            else//把imu作为输入，非point做法
            {
                // cout << "use_imu_as_input" << endl; 
                ...
                for (k = 0; k < time_seq.size(); k++)
                {
                    ...
                    {
                        /******* Publish odometry *******/
                        publish_my_pose(pub_my_pose);//1500HZ
                        publish_odometry(pubOdomAftMapped);
                        if (runtime_pos_log)
                        {
            ...
            if (!publish_odometry_without_downsample)
            {
                publish_my_pose(pub_my_pose);//10HZ
                publish_odometry(pubOdomAftMapped);
            }
